...
<head>
	...
	.invoice-logo.top_left {
		position: absolute;
		left: 0;
		 text-align: left; 
	}
	.invoice-logo.top_middle { left: 50%; transform: translateX(43%); text-align: center;}
	.invoice-logo.top_right { position: absolute; right: 0; text-align: right; }

	.invoice-logo.bottom_left { position: absolute; bottom: 0; left: 0; text-align: left; width: 100%; }
	.invoice-logo.bottom_middle { position: absolute; bottom: 10px; left: 50%; transform: translateX(-50%); text-align: center; }
	.invoice-logo.bottom_right { position: absolute; bottom: 0; right: 0; text-align: right; }



	.invoice-barcode.top_left    { text-align: left; }
	.invoice-barcode.top_middle  { text-align: center; }
	.invoice-barcode.top_right   { text-align: right; }

	.invoice-barcode.bottom_left   {margin-top: 25px; bottom: 10px; left: 0; text-align: left; width: 100%; }
	.invoice-barcode.bottom_middle {margin-top: 25px; bottom: 10px; left: 50%; transform: translateX(-50%); text-align: center; }
	.invoice-barcode.bottom_right  { margin-top:25px; bottom: 10px; right: 0; text-align: right; }
...
<div class="row" style="color: #000000;">
		...
		@if(empty($receipt_details->letter_head))
			...
			@if(!empty($receipt_details->logo))
				...
				<div class="invoice-logo {{ $receipt_details->logo_place ?? 'top_middle' }}">
					<img src="{{ $receipt_details->logo }}" alt="Logo" style="max-height: 120px; width: auto;" class="img img-responsive">
				</div>
			@endif
			...
			@if(!empty($receipt_details->header_text))
				<div class="col-xs-12" class="invoice-header-text">
...
<div class="row" style="color: #000000 !important;">
    ...
    @if(!empty($receipt_details->footer_image))
        <div class="col-xs-12 text-center mt-10">
            <img src="{{ $receipt_details->footer_image}}" 
                 alt="Footer Image" 
	...
	@if($receipt_details->show_barcode || $receipt_details->show_qr_code)
		@php
			...
			$barcode_place = $receipt_details->code_place ?? 'bottom_middle';
		@endphp

		<div class="invoice-barcode {{ $barcode_place }}
			@if(!empty($receipt_details->footer_text)) col-xs-4 @else col-xs-12 @endif">
